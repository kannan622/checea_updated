...
public class Handler_ {
    ...
    public String makeServiceCall_login(String reqUrl, String email, String password) {
        ...
        try {
            ...
            conn.setRequestProperty("version", Iconstant.versioncode);
    ...
    public String makeServiceCall_accesstoken_gettransaction(String reqUrl) {
      ...
      //  session_role = (session.getUserDetails().get(SessionManager.KEY_role));
      //  session_user_id = (session.getUserDetails().get(SessionManager.KEY_user_id));
        String response = null;
        try {
            URL url = new URL(reqUrl);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
